...
class PhotoProcessor {
  ...
  static bool needsProcessing({required bool mirror, String? watermark, double? aspect}) =>
  ...
  static Future<String> process(String imagePath, {required bool mirror, String? watermark, double? aspect}) async {
    if (!needsProcessing(mirror: mirror, watermark: watermark, aspect: aspect)) return imagePath;

    final tempDir = (await getTemporaryDirectory()).path;
    final outputPath = '$tempDir/aura_${DateTime.now().microsecondsSinceEpoch}.jpg';
    ...
    try {
      ...
      debugPrint('PhotoProcessor: native processing failed, using Dart fallback: $e');
    }
...
Future<String> _processInDart(Map<String, dynamic> args) async {
  ...
  try {
    ...
    img.Image? image = img.decodeJpg(bytes) ?? img.decodeImage(bytes);
    ...
    if (aspect != null) {
      ...
      cw -= cw % 2;
      ch -= ch % 2;
      image = img.copyCrop(image, x: (w - cw) ~/ 2, y: (h - ch) ~/ 2, width: cw, height: ch);
    }
